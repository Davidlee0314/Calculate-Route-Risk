...
void dodgeRisk(vector<Point>& turnsData, vector<Point>& splitsData, const Point& start,
               const Point& end, const vector<Circle>& circleData, int weight, double lineRisk, int distLim);
...
//    }
    dodgeRisk(turnsData, splitsData, start, end, circleData, weight, lineRisk, distLim);
    cout << turnsData.size();
...
void dodgeRisk(vector<Point>& turnsData, vector<Point>& splitsData, const Point& start,
    ...
    if((high.x - high.close.x) / (high.y - high.close.y) != (high.x - end.x) / (high.y - end.y)){
        ...
        toX = 20 * (high.x - high.close.x) / dist;
        toY = 20 * (high.y - high.close.y) / dist;
    }else{
        dist = sqrt(pow(end.x - high.close.x, 2) + pow(end.y - high.close.y, 2));
        toX = 20 * (high.close.y - end.y) / dist;
        toY = 20 * (end.x - high.close.x) / dist;
    }
    double distTurn;
    do{
        high.x += toX;
